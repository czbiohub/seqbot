...
def scan_dir(seq_dir, client, logger):
    ...
    file_set = {
        r["Key"] for result in response_iterator for r in result.get("Contents", [])
    }
    logger.info(
        "Found {} objects in {}".format(
...
def main(logger, upload_set):
    ...
    for seq in SEQS:
        ...
        logger.debug(
            "{} ~complete runs in {}".format(len(fns), os.path.join(ROOT_DIR, seq))
        )
        for fn in fns:
            ...
            if seq != "NovaSeq-01" or os.path.exists(
                os.path.join(seq_dir, "CopyComplete.txt")
            ):
                ...
                for root, dirs, files in os.walk(seq_dir, topdown=True):
                    ...
                    for file_name in files:
                        s3_key = os.path.join(S3_BCL_DIR, base_dir, file_name)
                        if s3_key not in file_set:
                            logger.debug("uploading {} to {}".format(file_name, s3_key))
                            try:
                                client.upload_file(
                                ...
                                logger.warning("couldn't read {}".format(file_name))
                            time.sleep(SLEEPY_TIME)

                    if synced_files:
                        logger.debug("synced {} files in {}".format(synced_files, root))
...
if __name__ == "__main__":
    ...
    info_handler = TimedRotatingFileHandler(info_log_file, when="W0", backupCount=10)
    info_handler.setLevel(logging.INFO)

    debug_log_file = "/home/utility/flexo_debug.log"
    debug_handler = TimedRotatingFileHandler(
        debug_log_file, when="midnight", backupCount=3
    )
    ...
    mainlogger.info(
        "synced {} new runs".format(len(updated_upload_set) - len(old_upload_set))
    )
